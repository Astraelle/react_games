...
const Fight2 = () => {
  ...
  const [playerHealth, setPlayerHealth] = useState(arrayStore1.stamina);
  const [ennemyHealth, setEnnemyHealth] = useState(arrayStore0.stamina);
  ...
  return (
    <div>
      ...
      <div>
        <p>{message}</p>
        <p>{ennemyHealth <= 0 ? `Bravo ! Vous avez vaincu ${arrayStore0.name} !` : ""}</p>
        <p>{playerHealth <= 0 ? `Dommage ! ${arrayStore0.name} Vous a vaincu!` : ""}</p>
      </div>
    </div>
  )
}
